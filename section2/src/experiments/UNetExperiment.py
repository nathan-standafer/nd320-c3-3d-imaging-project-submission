...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: The prediction has a shape of torch.Size([8, 3, 64, 64]).
            # The first dimension is the batch size. The second dimension is the number of classes.  ??????????????????????
            # The third dimension is the height of the volume. The fourth dimension is the width of the volume.
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            # code to view images for troubleshooting...            
            # actual_label = x["seg"]
            # for j in range(pred_label.shape[0]):
            #     plt.imshow(pred_label[j], cmap='gray')
            #     plt.savefig('img_pred{}.png'.format(j))

            #     plt.imshow(actual_label[j], cmap='gray')
            #     plt.savefig('img_actual{}.png'.format(j))
